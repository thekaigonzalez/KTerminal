...
def mainc(scr):
    ...
    while True:
        ...
        cmp += 1
        kt_argc = len(kt_argv)
        if (sys.argv.__contains__("--all-commands")):
                    ...
                    if bios == true:
                        ...
                        try:
                            if pathlib.Path("./" + kt_command + ".ktsh").exists():
                                ...
                                for i in lines:
                                    terminal.eval.Evaluatecommand(i, wd, stdscr, kt_argv)
                            elif pathlib.Path("./" + kt_command + ".sh").exists():
                                file = open('./' + kt_command + '.sh')
                                lines = file.readlines()
                                for i in lines:
                                    terminal.eval.Evaluatecommand(i, wd, stdscr, kt_argv)
                            elif pathlib.Path(kt_command).exists():
                            ...
                            deum.close()
        elif sys.argv.__contains__("--externalonly") and sys.argv.__contains__("--nopython") and sys.argv.__contains__("-nofs"): # External Commands only
            if kt_command == "help":
                arr = []
                for fname in os.listdir("usr/bin"):
                    if fname.endswith(".py"):
